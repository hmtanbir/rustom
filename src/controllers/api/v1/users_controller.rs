...
pub async fn index(
    ...
    Ok(Json(serde_json::to_value(paginated_res).unwrap()))
}
...
pub async fn update_me(
    State(state): State<AppState>,
    AuthenticatedUser(claims): AuthenticatedUser,
    Json(payload_wrapper): Json<UserPayloadWrapper<UserUpdateRequestDto>>,
) -> Result<Json<Value>, AppError> {
    let mut payload = payload_wrapper.into_inner();

    // Normal users cannot change their own role, ensure it is stripped
    if claims.role != 0 {
        payload.role = None;
    }
...
pub async fn create(
    State(state): State<AppState>,
    AuthenticatedUser(claims): AuthenticatedUser,
    Json(payload_wrapper): Json<UserPayloadWrapper<UserCreateRequestDto>>,
...
pub async fn update(
    ...
    // Strip role updates if user is not admin
    if claims.role != 0 {
        payload.role = None;
    }
